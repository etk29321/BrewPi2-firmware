...
public:
	...
	bool channelWrite(pio_t pio, bool set)
	{
		bool ok = false;
		uint8_t result = channelReadAll();
                //logInfoInt(INFO_ONEWIRE_READ, result);
#ifndef XCODE
		//bLink->printDebug("Read OneWire channel - 0x%x", result);
#endif
		if (result>=0) {
			...
			if (set)
				...
				result &= ~mask;
#ifndef XCODE
			//bLink->printDebug("Writing to OneWire channel - 0x%x", result);
#endif
			ok = channelWriteAll((uint8_t)result);
		}
                //logInfoIntInt(INFO_ONEWIRE_WRITE, result, ok);
#ifndef XCODE
		//bLink->printDebug("Completed channelWrite=%d", ok);
#endif
		return ok;
	}
	...
	bool channelWriteAll(uint8_t values) {
            bool ok=false;
            ok=accessWrite(values);
            //logInfoIntInt(INFO_ONEWIRE_WRITE, values, ok);
            return ok;
	}
};

//typedef class DS2413 DS2413;
