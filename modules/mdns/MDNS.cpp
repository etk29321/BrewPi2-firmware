...
bool MDNS::stop() {
	IPAddress mdnsdomain(224,0,0,251); // mDNS broadcast domain
    udp->leaveMulticast(mdnsdomain); // leave mDNS broadcast domain
    udp->stop();
    begun=false;
    return true;
}
...
int MDNS::processQueries() {
    ...
    if (n > 0) {
    	bLink->printDebug("%d:Got UDP Packet", millis());
        buffer->read(udp);
        ...
        if (buffer->available() >= 12) {
            ...
            bLink->printDebug("splunk/mdns/processQueries Query flags:0x%x qdcount:0x%x",flags,qdcount);
            ...
            if ((flags & 0x8000) == 0) {
                while (qdcount-- > 0) {
                    ...
                    if (buffer->available() >= 4) {
                        ...
                        bLink->printDebug("splunk/mdns/processQueries Query lastname:%s type:0x%x, cls:0x%x matchedName:%d",strbuf,type,cls,matchedName);
